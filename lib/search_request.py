...
class SearchRequest:
    ...
    def get_search_page(self):
        ...
        if self.search_type.lower() == "title":
            # &res=50 limits the number of results to 50
            # mirror : http://libgen.rs/search.php?
            search_url = (
                f"http://gen.lib.rus.ec/search.php?req={query_parsed}&column=title"
                # f"http://gen.lib.rus.ec/search.php?req={query_parsed}&res=50"
            )
        elif self.search_type.lower() == "author":
            search_url = (
                f"http://gen.lib.rus.ec/search.php?req={query_parsed}&column=author"
            )
        elif self.search_type.lower() == "auto":
            search_url = (
                f"http://gen.lib.rus.ec/search.php?req={query_parsed}&res=50"
            )
